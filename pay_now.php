...
if (isset($_POST['pay_now'])) {
  ...
  $num_rooms = $frm_data['num_rooms']; // Lấy số lượng phòng đã đặt
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    $new_quantity = $quantity - $num_rooms; // Trừ số lượng phòng đã đặt

    if ($new_quantity >= 0) { // Kiểm tra không để số lượng âm
      $query = "UPDATE rooms SET quantity = ? WHERE id = ?";
      update($query, [$new_quantity, $room_id], 'ii');
    } else {
      // Nếu không đủ phòng, có thể thêm xử lý lỗi
      echo "<h3 class='text-danger'>Không đủ phòng trống để đặt!</h3>";
      exit;
    }
  }
