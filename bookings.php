...
while ($data = mysqli_fetch_assoc($result)) {
  ...
  // Tính toán số ngày nếu trường 'days' không tồn tại
  ...
  if ($data['booking_status'] == 'Đã Thanh Toán') {
    ...
    $btn .= <<<form_button
<form method="POST" action="/khachsan/process_momo.php" target="_blank" style="display:inline;">
    <input type="hidden" name="booking_id" value="{$data['booking_id']}">
    <input type="hidden" name="price" value="{$data['price']}">
    <input type="hidden" name="days" value="{$days}">
    <input type="hidden" name="amount" value="{$data['total_pay']}">
    <button type="submit" class="btn btn-success btn-sm shadow-none ms-2">Thanh Toán</button>
</form>
form_button;
  } else {
